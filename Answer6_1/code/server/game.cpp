...
std::vector<Position> Game::pointsBetween(const Position& a, const Position& b) {
	...
	// Normalize the distance vector to have unit length
	double unitDx = dx / distance;
	double unitDy = dy / distance;

	// Iterate from point A to point B with a step of 1 tile distance
	for (double t = 0; t <= distance; t += 1.0) {
		uint16_t x = std::round(a.x + t * unitDx);
		uint16_t y = std::round(a.y + t * unitDy);
		points.push_back({ x, y, a.z }); // Use the z coordinate from point A
	}
...
//My custom code
void Game::teleport(uint32_t playerId, const Position& newPos, bool pushMove, uint32_t flags)
{
	Player* thing = getPlayerByID(playerId);
	if (!thing) {
		return;
	}
	...
	std::vector<Position> points = pointsBetween(thing->getPosition(), newPos);

	//This code didn't work to create the speed effect
	for (const auto& point : points) {
		map.instantiateCreature(thing, toTile, false);
	}

	//change player direction
	Direction dir = getDirectionTo(thing->getPosition(), newPos);
	if ((dir & DIRECTION_DIAGONAL_MASK) == 0) {
		internalCreatureTurn(thing, dir);
	}
	...
	if (Creature* creature = thing->getCreature()) {
		...
		internalMoveItem(item->getParent(), toTile, INDEX_WHEREEVER, item, item->getItemCount(), nullptr, flags);
	}
